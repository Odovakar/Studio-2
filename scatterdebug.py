...
ipv4_group_to_ticks = {
    '0-10k': 5000,        # Midpoint of the range as representative value
    '10k-100k': 55000,
    '100k-1M': 550000,
    '1M-10M': 5500000,
    '10M-100M': 55000000,
...
def update_scatter_plot(scale_type):
    def get_axis_type(scale_type):
        ...
        if scale_type == 'grouped':
            ...
            # For other scales, use default log scale settings
            scatter_fig.update_yaxes(type='log', title_text='Number of IPv4 Addresses (Log Scale)')

    if scale_type=='grouped':
    # Basic setup for the scatter plot remains the same
        scatter_fig = px.scatter(
            data_frame=json_df,
            x='pop',
            y='ipv4',
            hover_name="name",
            color='ipv4_grouping',  # Color by ipv4_grouping for visual grouping
            size_max=60,
        ...
        # Update marker sizes based on the full range of IPv4 addresses
        ...
        scatter_fig.update_traces(marker=dict(sizemode='area', sizeref=sizeref, sizemin=4))
            ...
            # Basic setup for the scatter plot
        scatter_fig = px.scatter(
            data_frame=json_df,
            x='pop',
            y='ipv4',
            hover_name="name",
            color='ipv4_grouping',  # Enhanced color coding
            size='ipv4',
            size_max=60,
            title='Population Size vs. Number of IPv4 Addresses by Country',
            labels={"pop": "Population Size", "ipv4": "Number of IPv4 Addresses"},
        ...
        if 'USA' in json_df['iso_alpha_3'].values:
            usa_ipv4 = json_df.loc[json_df['iso_alpha_3'] == 'USA', 'ipv4'].iloc[0]
            usa_pop = json_df.loc[json_df['iso_alpha_3'] == 'USA', 'pop'].iloc[0]
